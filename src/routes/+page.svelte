<script>
    export let data;
    let urls = data.urls; // Use server-loaded data
    let newUrl = '';
    let isMenuOpen = false; // For mobile menu toggle
  
    function addUrl() {
      if (newUrl.trim()) {
        urls = [...urls, newUrl.trim()];
        newUrl = '';
      }
    }
  </script>
    ...
    <div class="relative isolate px-6 pt-14 lg:px-8">
      ...
      <div class="mx-auto max-w-2xl py-32 sm:py-48 lg:py-56">
        <div class="text-center">
          ...
          <div class="mt-8">
            <form on:submit|preventDefault={addUrl} class="mb-8">
              <input
                type="url"
                bind:value={newUrl}
            ...
            {:else}
              <ul class="space-y-4">
                {#each urls as url}
                  <li
                    class="border p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors"
                  >
                    <a
                      href={url}
                      target="_blank"
